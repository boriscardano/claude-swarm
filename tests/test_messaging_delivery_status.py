...
class TestSingleMessageDeliveryStatus:
    ...
    def test_send_message_success_delivery_status(self, mock_get_pane, mock_send_to_pane, tmp_path):
    ...
    @patch("claudeswarm.messaging.TmuxMessageDelivery.send_to_pane")
    @patch("claudeswarm.messaging.MessagingSystem._get_agent_pane")
    def test_send_message_failure_delivery_status(self, mock_get_pane, mock_send_to_pane, tmp_path):
...
class TestBroadcastDeliveryStatus:
    ...
    def test_broadcast_all_success_delivery_status(
        ...
        assert results == {"agent-1": True, "agent-2": True, "agent-3": True}
    ...
    def test_broadcast_all_fail_delivery_status(
        ...
        assert results == {"agent-1": False, "agent-2": False}
    ...
    def test_broadcast_mixed_exceptions_delivery_status(
        ...
        mock_send_to_pane.side_effect = [True, TmuxPaneNotFoundError("Pane not found"), True]
...
class TestDeliveryStatusPersistence:
    ...
    @patch("claudeswarm.messaging.TmuxMessageDelivery.send_to_pane")
    @patch("claudeswarm.messaging.MessagingSystem._get_agent_pane")
    def test_message_log_includes_delivery_status(self, mock_get_pane, mock_send_to_pane, tmp_path):
    ...
    def test_broadcast_log_includes_delivery_status(
        ...
        assert "delivery_status" in log_entry
        assert log_entry["delivery_status"] == {"agent-1": True, "agent-2": False, "agent-3": True}
    ...
    def test_broadcast_log_counts_match_delivery_status(
        ...
        mock_registry = Mock()
        agents = [
            Mock(id=f"agent-{i}", pane_index=f"test:0.{i}", status="active") for i in range(1, 6)
        ]
...
class TestDeliveryStatusPublicAPI:
    ...
    def test_broadcast_message_function_returns_delivery_status(self, mock_broadcast):
        ...
        result = broadcast_message(
            sender_id="system", message_type=MessageType.INFO, content="Broadcast test"
        )

        assert result == {"agent-1": True, "agent-2": False, "agent-3": True}
