...
class TestAgentRegistry:
    ...
    def test_registry_creation(self):
        ...
        registry = AgentRegistry(
            session_name="main", updated_at="2025-11-07T12:00:00+00:00", agents=agents
        )
...
class TestAgentIdGeneration:
    ...
    def test_generate_agent_id_incremental(self):
        ...
        existing = {"main:0.0": "agent-0", "main:0.1": "agent-1", "main:0.2": "agent-2"}
...
class TestDiscoverAgents:
    ...
    def test_discover_agents_basic(
        self, mock_load, mock_parse, mock_is_in_project, tmp_path, monkeypatch
    ):
        ...
        mock_parse.return_value = [
            ...
            {"session_name": "main", "pane_index": "main:0.1", "pid": 1235, "command": "bash"},
            {
                "session_name": "main",
                "pane_index": "main:0.2",
                "pid": 1236,
    ...
    @patch("claudeswarm.discovery._parse_tmux_panes")
    @patch("claudeswarm.discovery._load_existing_registry")
    def test_discover_agents_preserve_ids(
        self, mock_load, mock_parse, mock_is_in_project, tmp_path, monkeypatch
    ):
        ...
        mock_load.return_value = AgentRegistry(
            session_name="main", updated_at="2025-11-07T12:00:00+00:00", agents=existing_agents
        )
    ...
    @patch("claudeswarm.discovery._parse_tmux_panes")
    @patch("claudeswarm.discovery._load_existing_registry")
    def test_discover_agents_stale_detection(
        self, mock_load, mock_parse, mock_is_in_project, tmp_path, monkeypatch
    ):
        ...
        mock_load.return_value = AgentRegistry(
            session_name="main", updated_at=old_time, agents=existing_agents
        )
